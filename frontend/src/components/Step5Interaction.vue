        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                <div class="tools-card-name">보고서 에이전트 채팅</div>
                <div class="tools-card-subtitle">보고서 생성 에이전트의 빠른 대화 모드입니다. 4가지 전문 도구를 호출할 수 있으며 MiroFish의 전체 기억을 보유합니다.</div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">현실 세계의 시드 데이터와 시뮬레이션 환경 상태를 정렬하고 Global/Local Memory 메커니즘을 결합해 시공간을 넘는 심층 분석을 제공합니다.</div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">전방위 추적</div>
                    <div class="tool-desc">그래프 구조 기반의 너비 우선 탐색 알고리즘으로 사건 전파 경로를 재구성하고 전체 정보 흐름의 토폴로지를 포착합니다.</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">GraphRAG 기반의 즉시 질의 인터페이스입니다. 인덱스 효율을 최적화해 구체적인 노드 속성과 개별 사실을 빠르게 추출합니다.</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">가상 인터뷰</div>
                    <div class="tool-desc">자율형 인터뷰로 시뮬레이션 세계의 개체들과 병렬로 여러 차례 대화하며 비정형 의견 데이터와 심리 상태를 수집합니다.</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? '보고서 에이전트와 대화하며 보고서 내용을 깊이 이해해 보세요' : '시뮬레이션 개체와 대화하며 그들의 관점을 살펴보세요' }}
              </p>
            </div>
            <div 
              ...
              <div class="message-content">
                <div class="message-header">
                  <span class="sender-name">
                    {{ msg.role === 'user' ? '사용자' : (chatTarget === 'report_agent' ? '보고서 에이전트' : (selectedAgent?.username || '에이전트')) }}
                  </span>
          ...
          <div class="survey-setup">
            ...
            <div class="setup-section">
              ...
              <textarea 
                ...
                class="survey-input"
                placeholder="선택한 모든 대상에게 물을 질문을 입력하세요..."
                rows="3"
              ></textarea>
            </div>
...
const chatHistoryCache = ref({}) // 모든 대화 기록을 캐시합니다: { 'report_agent': [], 'agent_0': [], 'agent_1': [], ... }
...
const sendToAgent = async (message) => {
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .slice(-6)
      .map(msg => `${msg.role === 'user' ? '질문자' : '에이전트'}: ${msg.content}`)
      .join('\n')
    prompt = `다음은 이전 대화입니다:\n${historyContext}\n\n이제 새 질문은 다음과 같습니다:\n${message}`
  }
  ...
  if (res.success && res.data) {
    // 올바른 데이터 경로: res.data.result.results는 객체 딕셔너리입니다
    // 형식: {"twitter_0": {...}, "reddit_0": {...}} 또는 단일 플랫폼 {"reddit_0": {...}}
    const resultData = res.data.result || res.data
    const resultsDict = resultData.results || resultData
    
    // 객체 딕셔너리를 배열로 변환하고 reddit 플랫폼 응답을 우선합니다
    let responseContent = null
    const agentId = selectedAgentIndex.value
    
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // reddit 플랫폼 응답을 우선하고, 그다음 twitter를 사용합니다
...
const submitSurvey = async () => {
  ...
  addLog(`설문 전송 대상: ${selectedAgents.value.size}명에게...`)
  ...
  try {
    ...
    // 형식: {"twitter_0": {...}, "reddit_0": {...}, "twitter_1": {...}, ...}
      ...
      addLog(`응답 ${surveyResults.value.length}개를 받았습니다`)
    } else {
      throw new Error(res.error || '요청 실패')
    }
  } catch (err) {
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`시뮬레이션 개체 ${profiles.value.length}명을 불러왔습니다`)
    }
  } catch (err) {
...
/* 왼쪽 패널 - 보고서 스타일(Step4Report.vue와 완전히 동일) */
.left-panel.report-style {
  width: 45%;
  min-width: 450px;
  background: #FFFFFF;
...
/* 순서 있는 목록 번호 수정 - CSS 카운터로 여러 ol을 연속 번호로 만듭니다 */
.message-text {

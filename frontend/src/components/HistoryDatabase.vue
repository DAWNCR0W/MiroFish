<template>
  ...
  >
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        <!-- 카드 상단: simulation_id와 기능 사용 가능 상태 -->
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="modal-section">
                ...
                <div class="modal-empty" v-else>연결된 파일이 없습니다</div>
              </div>
            </div>
            ...
            <div class="modal-divider">
              ...
              <span class="divider-text">시뮬레이션 다시보기</span>
              <span class="divider-line"></span>
            </div>
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">3단계 시뮬레이션 실행과 5단계 심층 상호작용은 실행 중에만 시작할 수 있어, 과거 기록으로 다시 재생할 수 없습니다</span>
            </div>
          </div>
        </div>
      </Transition>
...
let pendingState = null  // 실행 대기 중인 목표 상태를 기록합니다

// 카드 레이아웃 설정 - 더 넓은 비율로 조정합니다
...
const containerStyle = computed(() => {
  ...
  // 실제 필요한 높이를 계산합니다: 행 수 * 카드 높이 + (행 수-1) * 간격 + 약간의 하단 여백
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // 현재 행의 카드 수를 계산해 각 행이 중앙에 오도록 합니다
    ...
    const x = offset * 35
    // 시작 위치를 조정해 제목에 가깝게 두되 적당한 간격은 유지합니다
...
// 시뮬레이션 요구사항에서 제목을 생성합니다(앞 20자 사용)
...
// simulation_id 표시를 형식화합니다(앞 6자만 사용)
...
// 회차 표시를 형식화합니다(현재 회차/총 회차)
const formatRounds = (simulation) => {
  const current = simulation.current_round || 0
  const total = simulation.total_rounds || 0
  if (total === 0) return '시작 전'
  return `${current}/${total}회`
}
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // 실행 대기 중인 목표 상태를 갱신합니다(애니메이션 중 여부와 상관없이 최신 상태를 기록합니다)
        pendingState = shouldExpand

        // 이전 디바운스 타이머를 지웁니다(새 스크롤 의도가 이전 값을 덮어씁니다)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }

        // 애니메이션 중이면 상태만 기록하고 끝난 뒤 처리합니다
        if (isAnimating) return

        // 목표 상태가 현재 상태와 같으면 처리할 필요가 없습니다
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }

        // 디바운스로 상태 전환을 지연해 빠른 깜빡임을 방지합니다
        // 펼칠 때는 짧게(50ms), 접을 때는 더 길게(200ms) 지연해 안정성을 높입니다
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // 대기 중인 상태가 여전히 필요한지 확인합니다(이후 스크롤로 덮였을 수 있습니다)
          ...
          // 애니메이션이 끝나면 잠금을 해제하고 대기 중인 상태 변경이 있는지 확인합니다
          setTimeout(() => {
            isAnimating = false

            // 애니메이션이 끝난 뒤 새로 처리할 상태가 있는지 확인합니다
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // 너무 빠른 전환을 피하기 위해 잠시 더 지연한 뒤 실행합니다
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    {
      // 여러 임계값을 사용해 감지를 더 부드럽게 만듭니다
      threshold: [0.4, 0.6, 0.8],
      // rootMargin을 조정해 뷰포트 하단을 위로 줄이고, 더 많이 스크롤해야 펼쳐지도록 합니다
...
// 라우트 변화를 감시해 홈으로 돌아오면 데이터를 다시 불러옵니다
...
onMounted(async () => {
  // DOM 렌더링이 끝난 뒤 데이터를 불러오도록 보장합니다
  await nextTick()
  await loadHistory()

  // DOM 렌더링을 기다린 뒤 관찰자를 초기화합니다
  setTimeout(() => {
    initObserver()
  }, 100)
})

// keep-alive를 사용 중이면 컴포넌트가 활성화될 때 데이터를 다시 불러옵니다
onActivated(() => {
  loadHistory()
})

onUnmounted(() => {
  // Intersection Observer를 정리합니다
  if (observer) {
    observer.disconnect()
    observer = null
  }
...
/* CSS 배경 패턴으로 일정 간격의 정사각형 그리드를 만듭니다 */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* 왼쪽 상단을 기준으로 배치해 높이가 변해도 아래쪽만 확장되고 기존 그리드 위치는 유지합니다 */
...
.cards-container {
  ...
  /* min-height는 JS에서 동적으로 계산해 카드 수에 맞춰 조정합니다 */
}

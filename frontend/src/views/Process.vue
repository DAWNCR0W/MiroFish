    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          ...
          <div class="header-right">
            ...
            <div class="action-buttons">
                ...
                <button class="action-btn" @click="toggleFullScreen" :title="isFullScreen ? '전체 화면 종료' : '전체 화면으로 보기'">
        ...
        <div class="graph-container" ref="graphContainer">
            ...
            <div v-if="selectedItem" class="detail-panel">
              ...
              <div v-else class="detail-content">
                ...
                <div class="edge-relation">
                  ...
                  <span class="edge-type">{{ selectedItem.data.name || selectedItem.data.fact_type || '관련됨' }}</span>
                  <span class="edge-arrow">→</span>
                ...
                <div class="detail-row">
                  <span class="detail-label">라벨:</span>
                  <span class="detail-value">{{ selectedItem.data.name || selectedItem.data.fact_type || '관련됨' }}</span>
                </div>
                ...
                <div class="detail-row" v-if="selectedItem.data.invalid_at">
                  <span class="detail-label">무효 시점:</span>
                ...
                <div class="detail-row" v-if="selectedItem.data.expired_at">
                  <span class="detail-label">만료 시점:</span>
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">그래프 데이터를 불러오는 중...</p>
          </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-text">온톨로지 생성을 기다리는 중...</p>
      ...
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
        <div class="panel-header dark-header">
          ...
          <span class="header-title">구축 흐름</span>
        </div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  문서를 업로드하면 LLM이 문서 내용을 분석해 여론 시뮬레이션에 적합한 온톨로지 구조(엔티티 유형 + 관계 유형)를 자동 생성합니다.
                </div>
              </div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                ...
                <div class="relation-list">
                  ...
                  <div v-if="(projectData.ontology.relation_types?.length || 0) > 5" class="relation-more">
                    +{{ projectData.ontology.relation_types.length - 5 }}개 관계 더 보기...
                  </div>
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">인터페이스 설명</div>
                <div class="detail-content">
                  생성된 온톨로지를 바탕으로 문서를 분할한 뒤 로컬 그래프 저장소에 기록해 엔티티와 관계를 추출합니다.
                </div>
              </div>
              ...
              <div class="detail-section" v-if="graphData">
                ...
                <div class="build-result">
                  ...
                  <div class="result-item">
                    ...
                    <span class="result-label">관계 간선</span>
                  </div>
                  <div class="result-item">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                ...
                <div class="phase-api">다음 단계로 이동할 준비가 완료되었습니다</div>
              </div>
...
const selectedItem = ref(null) // 선택한 노드 또는 간선
...
const goToNextStep = () => {
  // TODO: 환경 설정 단계로 이동
  alert('환경 설정 기능을 개발 중입니다...')
}

const toggleFullScreen = () => {
  isFullScreen.value = !isFullScreen.value
  // 전환이 끝나면 그래프를 다시 렌더링합니다
  setTimeout(() => {
    renderGraph()
  }, 350) 
}
...
const initProject = async () => {
  ...
  if (paramProjectId === 'new') {
    // 새 프로젝트: store에서 업로드 대기 데이터를 가져옵니다
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = '업로드 대기 파일이 없습니다. /process/new 페이지를 새로고침했다면 홈에서 파일을 다시 선택해야 합니다.'
    loading.value = false
    return
  }
...
const updatePhaseByStatus = (status) => {
  switch (status) {
    ...
    case 'graph_completed':
      ...
      break
    case 'failed':
      error.value = projectData.value?.error || '처리에 실패했습니다'
      break
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      // 폴링에 사용할 task_id를 저장합니다
      const taskId = response.data.task_id
      
      // 그래프 데이터 폴링을 시작합니다(작업 상태 폴링과 분리)
      startGraphPolling()
      
      // 작업 상태 폴링을 시작합니다
      startPollingTask(taskId)
    } else {
      error.value = response.error || '그래프 구축 시작에 실패했습니다'
...
const fetchGraphData = async () => {
  try {
    // 먼저 graph_id를 얻기 위해 프로젝트 정보를 가져옵니다
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        // 진행률 표시를 완료 상태로 갱신합니다
        buildProgress.value = {
          progress: 100,
          message: '구축 완료, 그래프를 불러오는 중...'
        }
        
        // graph_id를 얻기 위해 프로젝트 데이터를 다시 불러옵니다
        const projectResponse = await getProject(currentProjectId.value)
        ...
        stopPolling()
        stopGraphPolling()
        error.value = '그래프 구축 실패: ' + (task.error || '알 수 없음')
        buildProgress.value = null
      }
    }
  } catch (err) {
...
const renderGraph = () => {
  ...
  const nodes = nodesData.map(n => ({
    ...
    rawData: n // 원본 데이터를 저장합니다
  }))
  
  // 유효한 간선을 거르기 위해 노드 ID 집합을 만듭니다
  ...
  simulation.on('tick', () => {
    // 모든 간선 위치를 갱신합니다(보이는 선과 투명 클릭 영역 포함)
